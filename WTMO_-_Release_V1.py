...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QProgressBar, QCheckBox, QSizePolicy, QTextBrowser
)
...
'''Okay before we start I will quickly give the mod organizer's basic overview/setup: The mod organizer opens and has a preset mod hosting website loaded for the user, this preset can be modified at any time (search: "self.url_input.setPlaceholderText" and replace the URL string) -> user then presses the root folders botton on the top left of the GUI, this will start a setup wizard
and within this wizard the user will select their file locations with the help of some hints. Once the wizard finishes, all categories will now have a route for deployment meaning the mod organizer is working. The user can now click on any handled mod they want to download and hit the "add mod" button in the top right of the GUI. The logic searches for the URL based on the
download URL string that any given mod hosting site uses, (these can be changed at "self.download_patterns"), it also grabs the first 16 characters of text on the page/post and will use that as the name for the mod (some mod hosting platforms have standardized titles which will make it easier, find the website's <div description segment or highlight the title text and right click to inspect and discover the desired text's location for the mod organizer to parse)
and finally a category is given based on the mod hosting site's category structure, I recommend looking at the page URL or looking at the mod tags for possible filters for your categories, in this case there are 4 cats: Camoflauges, Missions, Sights, and Controls ("Controlss", note that the cats may reference something that could be considered a typo, 
make absolutely certain you checked the spelling, it needs to match up, as in this case "Controls" are "Controlss", reference "category_patterns" for changes), once the organizer has this info it is displayed in the GUI in the mod downloads list. Once a user is happy with their selections, they hit the "download all" button in the bottom right and the organizer 
will download and deploy mods to their given locations based on their cats, note that the logic for these file structures needs to be studied: what folder structure does a game use, what does it look like if the mod author uploaded purely loose files, if it's a game with file chunks that go to differnt locations you must account for and route based on those needs, download some mods and walk through the installation process by hand and write it down, then replicate it with your code, make adjustments here: "def run", and "def _unpack_archive"
Once the download finishes up the code will make sure to add the Category, Name and URL to the master list which is a running tab of all mods the user downloads, the user can export the list via the export button in the top left and import via the import button in the top left, the user can then redownload all or whichever mods they would like by leaving a/multiple mods checkmarked in the mod list on the right. Please make any issues known on github: https://github.com/RuseTigerDev/War-Thunder-Mod-Organizer I will try to respond as best as I can.'''
...
class DownloadThread(QThread):
    ...
    finished_download = pyqtSignal(object, bool, str)  # mod, success, message
    all_done = pyqtSignal()
    ...
    def run(self):
        for i, mod in enumerate(self.mods):
            ...
            try:
                ...
                self.finished_download.emit(mod, True, f"Downloaded: {filename}")
            except Exception as e:
                self.finished_download.emit(mod, False, str(e))
...
CATEGORY_USER_CONTROLS = "user_controls"
VALID_CATEGORIES = {CATEGORY_CAMO, CATEGORY_MISSION, CATEGORY_SIGHT, CATEGORY_USER_CONTROLS}
...
class ModOrganizer(QMainWindow):
    def __init__(self):
        ...
        self.has_control_mods = False  # Track if controls were downloaded (these two checks are for NOTICE flags in the GUI, please make sure you add some if you modify the code for another game, they can be a big help for users)

        # Mod folder paths (set after root folder selection)
        self.user_skins_folder = ""
        self.user_missions_folder = ""
        self.user_controls_folder = ""
    ...
    def init_ui(self):
        ...
        This is a known issue, it will remain'''
        # === MAIN CONTENT AREA ===
        ...
        # Build/use the full path relative to the mod organizer location... I hope...
        image_path = resource_path("WarThunderModManagerFullLogoColor.png")
        pixmap = QPixmap(str(image_path))
        # Note that resource path in PyQt6 wants str or byte pathes not just a path, declare it, in our case it will be a str
        
        # 2. Check if the image loaded successfully
        if not pixmap.isNull():
            # 3. Scale the image to fit nicely without stretching
            # Adjust 280 (max width) and 160 (max height) to your desired logo size
            ...
            # Fallback text if the image fails to load (great for debugging, if you ever create your own image (which you may decide to do) hopefully the following text will greet you once or twice as it has greeted me 0_o )
            self.logo_label.setText(f"Game Logo\n(Image not found at:\n{image_path})")
        ...
        # Note: In PyQt6, strict enum usage may require .value for bitwise operations:
        # self.tools_frame.setFrameStyle(QFrame.Shape.Box.value | QFrame.Shadow.Sunken.value)
        self.tools_frame.setFrameStyle(QFrame.Shape.Box | QFrame.Shadow.Sunken)
        
        tools_layout = QVBoxLayout(self.tools_frame)
        
        # 1. Use QTextBrowser instead of QTextEdit for read-only rich text with links
        self.tools_text = QTextBrowser()
        
        # 2. Enable opening external links in the default web browser
        self.tools_text.setOpenExternalLinks(True)
        
        # 3. Use HTML to ensure clean formatting and a clickable link
        info_text = (
            "<p>Donation links, guides, or other tools. \nExists for text information, no images.</p>"
            "<p>Please check my github:<br>"
            "<a href='https://github.com/RuseTigerDev/War-Thunder-Mod-Organizer'>"
            "https://github.com/RuseTigerDev/War-Thunder-Mod-Organizer</a></p>"
        )
        self.tools_text.setHtml(info_text)
        ...
        self.tools_text.setPlaceholderText("Donation links, guides, or other tools. exists for text information, no images. Please check my github: https://github.com/RuseTigerDev/War-Thunder-Mod-Organizer")
    ...
    def find_root_folder(self):
        ...
        if folder:
            ...
            # Create/find UserControls folder
            self.user_controls_folder = os.path.join(folder, "UserControls")
            os.makedirs(self.user_controls_folder, exist_ok=True)
            # ----------------------------
            ...
            if prod_folder:
                ...
                # Create/find UserSights and all_tanks subfolder. all_tanks is the general folder for the game, that being said not all sights are meant for all tanks.
    ...
    most sights being loose .blk's and others sights specific to certain tanks being properly labeled and foldered. Could be a way to make it more robust, might need to DM Magazine: FIXED
    ...
    in coralation with the download of the mission mod(s): FIXED
    6. Mission mods do not deploy even when misscategorized, could be an issue with the loose .blk's in general, might effect Sights: FIXED (Update, issue persists regardless of category)
    7. Need to find a way to make loose camo mods install to a folder sharing the name of their zip folder, methods attempted result in unnescessary subfolders: FIXED 
     - Delete this '''
    ...
    def _extract_mod_name(self, html: str) -> Optional[str]:
        """Grab the first 16 characters of the mod description text, if present."""
        # Prefer the opened lightbox post (id="clb" with display: block)
        clb_match = re.search(
            r'<div id="clb"[^>]*display:\s*block[^>]*>(.*)\Z',
            html, re.IGNORECASE | re.DOTALL
        )
        area = clb_match.group(1) if clb_match else html

        # Find the opening <div class="description">
        desc_open = re.search(
            r'<div\s+class=["\']?description["\']?[^>]*>',
            area, re.IGNORECASE
        )
        if not desc_open:
            return None

        start_idx = desc_open.end()
        pos = start_idx
        depth = 1

        # Walk forward counting <div> vs </div> to find the matching close tag
        while depth > 0:
            m = re.search(r'<(/?)div\b', area[pos:], re.IGNORECASE)
            if not m:
                return None  # malformed HTML

            tag_start = pos + m.start()
            tag_end = area.find('>', tag_start)
            if tag_end == -1:
                return None
            ...
            if m.group(1):          # </div>
                depth -= 1
                if depth == 0:
                    # This is the closing tag for description
                    raw = area[start_idx:tag_start]
                    break
            else:                   # <div>
                depth += 1
            pos = tag_end + 1

        # Pull out ONLY <p> tags — ignores buttons, titles, info divs, etc. There can be a ton of noise to cut through, depends on the hosting platform.
        p_tags = re.findall(r'<p[^>]*>(.*?)</p>', raw, re.IGNORECASE | re.DOTALL)
        if not p_tags:
            return None
        ...
        if not text:
            return None
        return text[:16]
        # You can edit this section if desired to accomodate a more limited text parsing if your hosting site has, for example a strict nameing scheme and location on the page
    ...
    def _process_html_for_downloads(self, html: str):
        ...
        category_patterns = [
            ...
            (r'href="/feed/controlss/"', CATEGORY_USER_CONTROLS), 
        ]
        ...
        if category is None:
            # this segment is legacy code, if your game needs more cat logic, you might build it here, for example cat logic that finds the cat by look at tags that should only exist for 1 cat type, character presets/cards for example shouldn't have guns or food items and thus could recieve a cat that routes them to where ever a game saves their character presets even if the mod was posted without a good normal category
...
#                    "Incorrect Mod Class, Mod Organizer only handles Camouflage, Mission and Sight mods, sorry.")
            QMessageBox.warning(self, "Incorrect Mod Category",
                "Incorrect Mod Category, Mod Organizer only handles Camouflage, Mission, User Controls, and Sight mods, sorry. Please click on a handled mod post then click add mod")
            return
        ...
        if found_urls:
            mod_name = self._extract_mod_name(html)   # may be None
            for url in found_urls[:1]:
                self._add_mod_to_list(url, category, mod_name)
    ...
    def _add_mod_to_list(self, url: str, category: Optional[str] = None, custom_name: Optional[str] = None):
        """Add a mod URL to the download list with category and optional parsed name."""

        if custom_name:
            mod_name = custom_name
            mod_save_name = custom_name
        else:
            # Fallback: last non-empty path segment (WT Live URLs end in a
            # trailing slash, so split('/')[-1] alone would be an empty string)
            path_parts = [p for p in urlparse(url).path.split('/') if p]
            fallback = path_parts[-1] if path_parts else f"Mod {len(self.mod_list)+1}"
            mod_name = fallback
            mod_save_name = fallback
        ...
        if category == CATEGORY_CAMO:
            ...
            cat_prefix = "[SIGHT] "
        elif category == CATEGORY_USER_CONTROLS:
            cat_prefix = "[CONTROLS] "
        ...
        item.setData(Qt.ItemDataRole.UserRole, {'url': url, 'category': category, 'name': mod_name})
        ...
        for i in range(self.mod_listwidget.count()):
            ...
            if item.checkState() == Qt.CheckState.Checked:
                ...
                if category == CATEGORY_CAMO:
                    ...
                    target = self.user_controls_folder
                    self.has_control_mods = True
                

                mod_save_name = data.get('name') if isinstance(data, dict) else None

                mods_to_download.append({'url': url, 'target': target, 'category': category, 'name': mod_save_name})
    ...
    def _on_download_finished(self, mod, success: bool, message: str):
        ...
        if success:
            ...
            # Prevent duplicate entries - compare by URL so a re-downloaded
            # mod with a slightly different name doesn't get listed twice
            existing_urls = [(m.get('url') if isinstance(m, dict) else m) for m in self.master_list]
            if entry['url'] not in existing_urls:
                self.master_list.append(entry)

            self.statusBar().showMessage(f"Downloaded: {entry['url']}", 5000)

        else:
            self.statusBar().showMessage(f"Download failed: {message}", 5000)
    ...
    def _on_all_downloads_done(self):
        ...
        # Show user controls notification if any control mods were downloaded
        if self.has_control_mods:
            controls_folder = getattr(self, "user_controls_folder", "")

            if not controls_folder and getattr(self, "root_folder", ""):
                controls_folder = os.path.join(self.root_folder, "UserControls")

            message = (
                "Please note: Your control schemes are loaded manually in game. "
                "Reload the game, navigate to settings, controls, and click the "
                "import icon on the bottom left of your screen.\n\n"
                "All control mods are saved in UserControls folder."
            )

            if controls_folder:
                message += f"\n\nFolder location:\n{controls_folder}"

            QMessageBox.warning(self, "Control Mods Notice", message)
        #if self.has_control_mods:
            #QMessageBox.warning(self, "Control Mods Notice",
            #    "Please note: Your control schemes are loaded manually in game. "
            #    "Reload the game, navigate to settings, controls, and click the" 
            #    "import icon on the bottom left of your screen.\n" 
            #    "All control mods are saved in UserControls folder")
        ...
        if self.has_sight_mods:
            QMessageBox.warning(self, "Sight Mods Notice",
                ...
                "Also try to use the reload custom sights keybind in game: Alt+F9.")
    ...
    def show_full_modlist(self):
        ...
        # Group for display
        camos, missions, sights, user_controls = [], [], [], []
    ...
    def export_modlist(self):
        ...
        if not filepath:
            return
        groups = {CATEGORY_CAMO: [], CATEGORY_MISSION: [], CATEGORY_SIGHT: [], CATEGORY_USER_CONTROLS: []}
        unknown = []
        '''camos = []
        missions = []
        sights = []
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            for header, entries in [("[CAMO]", groups[CATEGORY_CAMO]),
                                    ("[MISSION]", groups[CATEGORY_MISSION]),
                                    ("[SIGHT]", groups[CATEGORY_SIGHT]),
                                    ("[USER_CONTROLS]", groups[CATEGORY_USER_CONTROLS]),
                                    ("[UNKNOWN]", unknown)]:
                if entries:
                    f.write(f"{header}\n")
                    for name, url in entries:
                        if name:
                            f.write(f"{name}\n")   # name line only when we have one
                        f.write(f"{url}\n") 
            '''if camos:
                f.write("[CAMO]\n")
                for url in camos:
            ...
            if user_controls:
                f.write("[USER_CONTROLS]\n")
                for url in user_controls:
                    f.write(f"{url}\n")
            ...
            try:
                ...
                if filepath.endswith('.txt'):
                    ...
                    for line in content.strip().split('\n'):
                        ...
                        elif line == "[USER_CONTROLS]":
                            current_category = CATEGORY_USER_CONTROLS

                        elif line == "[UNKNOWN]":
                            current_category = None
                        elif line.startswith('http'):
                            if line not in [m['url'] for m in self.mod_list]:
                                self._add_mod_to_list(line, current_category, pending_name)
                                imported_count += 1
                            pending_name = None  # always reset, even on duplicates
                        else:
                            pending_name = line  # it's a name; attach it to the next URL
                else:
                    # Legacy JSON format
                    with open(filepath, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                    for mod in data.get('mods', []):
                        ...
                        if url not in [m['url'] for m in self.mod_list]:
                            self._add_mod_to_list(url, category, name)
                            imported_count += 1
        ...
        if settings_path.exists():
            try:
                ...
                if self.root_folder:
                    ...
                    self.user_controls_folder = os.path.join(self.root_folder, "UserControls")
...
def main():
    ...
    app.setWindowIcon(QIcon(str(resource_path("WTMO.ico"))))
